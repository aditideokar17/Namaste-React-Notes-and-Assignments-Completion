    ...
    return(
        ...
        {items.map((item)=>(
            <div key={item.card.info.id} className=" bg-gray-50 shadow-lg rounded-lg p-2 m-2 ">
                ...
                <img src={CDN_URL + item?.card?.info?.imageId} className="w-[100px] h-20 shadow-sm rounded-sm" alt="food image"/><button className="ml-[520px] mt-14 px-2 bg-white text-green-500 rounded-sm font-semibold shadow-lg absolute" onClick={() => handleAddItem(item)}>Add +</button></div>
            </div>
        ))}
        </>
    )
